calcPCClocks <- function(path_to_PCClocks_directory, datMeth, datPheno){
  ...
  # JG Edits to code 
  # no more for loop, just merge in dataframe
  impute.df <- data.frame(t(imputeMissingCpGs[missingCpGs]))
  datMeth[, missingCpGs] <- impute.df

  #fill in CpGs that are NA for all values
  #fill in CpGs that are NA for all values
  all_missingCpGs <- CpGs[apply(datMeth[,CpGs], 2, function(x)all(is.na(x)))]
  # Hopefully won't throw an error anymore in event that there are no missing
  datMeth[, all_missingCpGs] <- impute.df[,all_missingCpGs]
